<?php 
echo "this is my index.php updated";
 
?>

<!DOCTYPE html> 
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=,initial-scale=1.0">
    <link rel="stylesheet" href="CSS/style.css">
    <!-- Fonts -->
    <link rel="preconnect" href="https://fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css2?family=Montserrat&display=swap" rel="stylesheet">

    <title>VEGAN FREEDOM</title>
</head>
<body>
    <nav>
        <div class="top"></div>
        <ul>
            <li><a href="#">HOME</a></li>
            <li><a href="php/blog.php">BLOG</a></li>
            <li><a href="#"><img src="logo/peace-symbol.svg" alt="peace symbol vegan" fill="white"></a></li>
            <li><a href="php/events.php">EVENTS</a></li>
            <li><a href="php/sign_in.php">SIGN IN</a></li>
        </ul>
    </nav>
    <header>
        <img src="img/go_vegan.png" alt="Go Vegan Schriftzug mit Brokkoli als Hintergrund in den Buchstaben">
    </header>
    <main>
        <div class="containerleft">
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
        </div>
        <aside class="containerright">
            <div class="kiste">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p></div>
                    </div>
                    <a href=""><button>Mehr...</button></a>
                </div>
            </div>
            <div class="kiste">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p></div>
                    </div>
                    <a href=""><button>Mehr...</button></a>
                </div>
            </div>
        </aside>
    </main>

    <div class="slider">
        <h1>Kategorien</h1>
        <span>

        </span>

    </div>

    <main>
        <div class="containerleft">
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
            <div class="block">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p>
                    </div>
                        <a href=""><button class="button">Mehr...</button></a>
                </div>
            </div>
        </div>
        <aside class="containerright">
            <div class="kiste">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p></div>
                    </div>
                    <a href=""><button>Mehr...</button></a>
                </div>
            </div>
            <div class="kiste">
                <div class="row">
                    <img src="img/gemüsespiess.jpg" alt="Zucchini-Karotten-Paprika">
                    <div class="text">
                        <h2>Vegane Spiesse</h2><p>Lipsum dolor sit amet consectetur adipisicing elit. Optio doloribus error ratione facilis fugiat impedit doloremque mollitia aspernatur exercitationem dicta illum vero blanditiis sint inventore enim, corrupti, accusantium quam dolore.</p><br><p>Author: Martin Hutchings</p></div>
                    </div>
                    <a href=""><button>Mehr...</button></a>
                </div>
            </div>
        </aside>
    </main>

    <footer>
        <img src="data:image/svg+xml;base64," />
    </footer>
    <div class="bottom"></div>

</body>
</html>